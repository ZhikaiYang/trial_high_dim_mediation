...
fwrite(as.data.frame(snp_list_eqtl), "largedata/snp_list_eqtl.txt", row.names = F, col.names = F, sep = "\t" )
...
##Qi ask all snps including non-sig within gwas peak

awk '$1==3 && $4>=160429526 && $4<=161478267' hmp321_282_agpv4_maf005_miss03.bim > gwas_peak.bim

awk '{ print $2}' gwas_peak.bim > snp_list_all.txt

#subset SNPs
plink -bfile  hmp321_282_agpv4_maf005_miss03 --extract snp_list_all.txt --make-bed --out hmp321_282_agpv4_maf005_miss03_new_mediation_all
##Convert binary file to VCF using plink
plink --bfile hmp321_282_agpv4_maf005_miss03_new_mediation_all --recode vcf-iid --out hmp321_282_agpv4_maf005_miss03_new_mediation_all
##Convert VCF to intermediate file using perl
perl subs_NA_0_mediation.pl
...
d=fread("hmp321_282_agpv4_maf005_miss03_new_mediation_all.vcf1", header=F,data.table=F)
d1=t(d)
mk=colnames(d1)=d1[1,]
colnames(d1)[1]="Genotype"
d1=d1[-1,]
...
#DNA Qi ask all
out="mediation_snp_matrix_all.txt"
fwrite(d4[,c(1,3:ncol(d4))],"mediation_snp_matrix_all.txt",sep="\t",col.names = T,row.names = F,quote=F)
...
zea_gene_anno = fread("largedata/Zea_mays.AGPv4_anno.txt", header = T , data.table = F)
gene_coordinate = merge(v3_v4[,1:2], zea_gene_anno[,1:4], by.x = "V2", by.y = "ensembl_gene_id_v4", all.x = T)
colnames(gene_coordinate)[1:2] = c("V4_gene_name", "V3_gene_name") 
fwrite(gene_coordinate, "largedata/gene_coordinate.txt", row.names = F, sep = "\t")
```
